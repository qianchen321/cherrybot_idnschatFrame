...
class Websocket:
    ...
    def websocketConnection(self):
        ...
        print(ConsoleTools.console_colour('INFO'), "获取连接状态:", ws_status)
        return ws_status

    def sendFirstRequest(self, first_request):
        self.ws.send(first_request)  # 发送请求参数
        self.first_answer = self.ws.recv()
        print(ConsoleTools.console_colour('INFO'), "返回:", self.first_answer)
    ...
    def getMessage(self):
        ...
        if cut_message is None:
            ...
            print(ConsoleTools.console_colour('Receive'), cut_message[1])
    ...
    def cutMessage(self, n):
        if n == 1:
            ...
            received_message = json.loads(str(self.received_message2))
        else:
            return None
        ...
        if "message" in received_message:
            ...
            time = TimeTools.TimeStampToTime(time_stamp13)  # 时间戳转时间
    ...
    def sendMessage(self, text):
        request = '{"type" : "message","group" : "idns_cn","name" : ' + self.name + ',"text" : "' + text + '","date" : ' + str(
            TimeTools.getTimeStamp()) + '}'
        # print(request)
        self.ws.send(request)
        print(f"{ConsoleTools.console_colour('Send')}{'['+str(datetime.datetime.now())+']:'}已发送: \n{text}\n")
